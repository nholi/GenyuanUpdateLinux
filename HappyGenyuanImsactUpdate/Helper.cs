...
namespace HappyGenyuanImsactUpdate
{
    ...
    public class Helper
    {
        ...
        public static void CheckForTools(bool requirediff = false)
        {
            ...
            if (!File.Exists($"{exePath}/hpatchz"))
            {
        ...
        public static bool AnyCertainGameExists(string checkdir)
        {
            foreach (var certaingame in Helper.certaingames)
            {
                if (File.Exists($"{checkdir}/{certaingame}.exe")) return true;
                if (File.Exists($"{checkdir}/{certaingame}Beta.exe")) return true;
            }
            return false;
        }
